...
waveShaper = bypass1(c,out)
  	with {
  		out(x) = db2linear(gain)*g(dist)*f(db2linear(dist),x);
	};

// One branch
effectBranch = bypass1(b,wahEffect) : waveShaper : bypass1(a,echoFunction);

distr = _,_ <: ((1-pan)*effectBranch, (pan)*effectBranch : *(wet),*(wet)), *(1-wet), *(1-wet) :> *(master), *(master)
	with {
		out(x) = db2linear(gain)*g(dist)*f(db2linear(dist),x);
